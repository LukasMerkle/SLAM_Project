...
from geometry_utils import transform_plane_to_world, transform_plane_to_local, \
                           compute_world_pose, rot3D, computeH, theta_to_normal
import copy
import scipy.linalg
import math
...
class SLAMBackend:
    ...
    def __init__(self, std_p, std_x, std_l, init_pose):
        ...
        self.landmarks = [] # num_landmark_measurements x 2
        self.std_p = std_p
        self.std_x = std_x
        self.std_l = std_l

    def landmark_model(self, pose_w, planes):
        transformed_plane = transform_plane_to_local(pose_w, planes)
        theta = np.arctan2(transformed_plane[1], transformed_plane[0])
        return np.hstack([theta, transformed_plane[-1]])
    ...
    def initalize_landmark(self, new_measurements):
        planes = np.hstack([theta_to_normal(new_measurements[:,0]), new_measurements[:,2:]])
        return transform_plane_to_world(self.s_x[-1], planes)
    ...
    def add_landmark_measurement(self, landmark_measurement):
        if self.s_l is not None:
            ...
            if new_indices.shape[0] > 0:
                ...
                self.s_l = np.vstack([self.s_l, self.initalize_landmark(new_measurements)])
        else:
            self.s_l = self.initalize_landmark(landmark_measurement)
...
if __name__ == "__main__":
    ...
    std_l = np.array([0.01, 0.01]) # theta, d
